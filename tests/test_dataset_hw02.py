...
from src.preprocessing.subtile_esd_hw02 import Subtile, TileMetadata, SatelliteMetadata  # Import your actual module where Subtile and TileMetadata are defined
...
class TestDSE(unittest.TestCase):
    def setUp(self):
        ...
        self.root_dir = root/"data/processed/Train1x1/subtiles/" #"/path/to/your/data"
        self.selected_bands = {"sentinel1": ["VV"], "gt": ["0"]}
        self.transform = None  # You can set a transformation function if needed
        self.dse_instance = DSE(root_dir=self.root_dir, selected_bands=self.selected_bands, transform=self.transform)

    def test_len_method(self):
        # Test the __len__ method
        self.assertEqual(len(self.dse_instance), len(list(Path(self.root_dir).glob("*.npz"))), 'The length of the dataset is not correct.')
    ...
    def test_aggregate_time_method(self):
        ...
        self.assertEqual(output_data.shape, expected_shape, 'The __aggregate_time method is not working as expected.')
        if self._outcome.success:
            global CURRENT_POINTS_TEST
            CURRENT_POINTS_TEST += 5
        print(f"CURRENT_POINTS_TEST: {CURRENT_POINTS_TEST}/{MAX_POINTS_TEST}")
    ...
    def test_select_indices_method(self):
        ...
        self.assertEqual(indices, expected_indices, 'The __select_indices method is not working as expected.')
        if self._outcome.success:
            global CURRENT_POINTS_TEST
            CURRENT_POINTS_TEST += 5
        print(f"CURRENT_POINTS_TEST: {CURRENT_POINTS_TEST}/{MAX_POINTS_TEST}")
    ...
    def test_select_bands_method(self):
        ...
        satellitemetadata_to_fill_tilemetadata_satellites_dict = {
            ...
            "viirs": SatelliteMetadata(
                ...
                bands=["0"],
                times=["20200221", "20200224", "20200225",
                          "20200226", "20200227", "20200231",
                          "20200235", "20200236", "20200237"],
                original_filenames=[
                    [
        ...
        subtile = Subtile(
            satellite_stack=subtile_satellite_stack,
            tile_metadata=subtile_tilemetadata
        )
        selected_sat_stack, selected_metadata = self.dse_instance._DSE__select_bands(subtile)
        ...
        self.assertEqual(selected_sat_stack["sentinel1"].shape, (4,1,100,100), "__select_bands method is not filtering by bands.")
        self.assertEqual(selected_metadata.satellites["sentinel1"].bands[0], 'VV', "__select_bands method is not filtering by bands.")
        if self._outcome.success:
            global CURRENT_POINTS_TEST
            CURRENT_POINTS_TEST += 5
        print(f"CURRENT_POINTS_TEST: {CURRENT_POINTS_TEST}/{MAX_POINTS_TEST}")
    ...
    def test_getitem_method(self):
        ...
        self.assertIsInstance(X, (np.ndarray, torch.Tensor), 'The __getitem__ method is not returning the expected type for X')
        self.assertIsInstance(y, (np.ndarray, torch.Tensor), 'The __getitem__ method is not returning the expected type for y')
        self.assertIsInstance(tile_metadata, TileMetadata, 'The __getitem__ method is not returning the expected type for tile_metadata') 
        if self._outcome.success:
            global CURRENT_POINTS_TEST
            CURRENT_POINTS_TEST += 5
        print(f"CURRENT_POINTS_TEST: {CURRENT_POINTS_TEST}/{MAX_POINTS_TEST}")
