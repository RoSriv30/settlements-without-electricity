...
class Subtile:
    ...
    def save(self, dir: str | os.PathLike):
        ...
        np.save(dir / "subtiles" / f"{self.tile_metadata.parent_tile_id}_{self.tile_metadata.x_gt}_{self.tile_metadata.y_gt}.npz", **self.satellite_stack)
...
def get_tile_ground_truth(
    ...
    if type(size_gt) == type(1):
        size_gt = (size_gt,size_gt)

    # calculate the start and end indices for the slice based
    # on the size_gt and the x and y coordinates
        
    start_x, start_y =  (size_gt[0]*x_gt, size_gt[1]*y_gt)
    end_x, end_y =  (size_gt[0]*(x_gt+1), size_gt[1]*(y_gt+1))
    # check to make sure the slice is within the bounds of
    # the ground truth parent image
    x_bound, y_bound = gt_parent.shape[2], gt_parent.shape[3]

    if start_x < 0 or start_y < 0 or end_x > x_bound or end_y > y_bound:
        raise ValueError('Slice is not within bounds of gt_parent')
...
def get_tile_satellite(
    ...
    # calculate the start and end indices for the slice based
    # on the size_gt and the x and y coordinates
    start_x, start_y = (scale_factor*size_gt[0]*x_sat, scale_factor*size_gt[1]*y_sat)
    end_x, end_y = (scale_factor*size_gt[0]*(x_sat+1), scale_factor*size_gt[1]*(y_sat+1))
...
def grid_slice(
    ...
    ground_truth_stack = satellite_stack['gt']
    scale_factor = 50

    if ground_truth_stack.shape[2] % tile_size_gt != 0 or ground_truth_stack.shape[3] % tile_size_gt != 0:
        raise ValueError("Tile size is not compatible with ground truth stack dimensions.")

    num_tiles_x = ground_truth_stack.shape[2] // tile_size_gt
    num_tiles_y = ground_truth_stack.shape[3] // tile_size_gt
    ...
    for x in range(num_tiles_x):
        for y in range(num_tiles_y):
            satellite_tiles = {}
            for satellite_type, satellite_data in satellite_stack.items():
                if satellite_type == 'gt':

                    tile_data = get_tile_ground_truth(satellite_data, x, y, tile_size_gt)
                else:
                    tile_data = get_tile_satellite(satellite_data, x, y, tile_size_gt, scale_factor)
                satellite_tiles[satellite_type] = tile_data
...
def restitch(
    ...
    # Initialize lists to hold stitched data and metadata
    stitched_data = []
    stitched_metadata = []
    ...
    for x in range(range_x[0], range_x[1]):
        ...
        for y in range(range_y[0], range_y[1]):
            # Load subtile data
            subtile = Subtile().load(os.path.join(dir, f"{tile_id}_{x}_{y}.npz"))
            subtile_data = subtile.satellite_stack[satellite_type]

            # Load subtile metadata
            subtile_metadata = subtile.tile_metadata

            row_data.append(subtile_data)
            row_metadata.append(subtile_metadata)

        stitched_data.append(np.concatenate(row_data, axis=3))
        stitched_metadata.append(row_metadata)

    stitched_image = np.concatenate(stitched_data, axis=2)

    return stitched_image, stitched_metadata
